...
class _LoginScreenState extends State<LoginScreen> {
  ...
  bool get _canAdvance {
    switch (_step) {
      case 'phone': return _phone.length == 10;
      case 'otp': return _otp.length == 6;
      default: return true;
    }
  }
                ...
                Row(
                  children: [
                    GestureDetector(
                      onTap: () {
                        if (_stepIdx > 0) {
                          setState(() { _step = _steps[_stepIdx - 1]; });
                        } else {
  ...
  Widget _buildLangStep() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        KsText(t('chooseLanguage'), style: GoogleFonts.notoSans(fontSize: 24, fontWeight: FontWeight.w700)),
        const SizedBox(height: 4),
        KsText(t('chooseLanguageSub'), style: GoogleFonts.notoSans(fontSize: 14, color: AppTheme.mutedForeground)),
        const SizedBox(height: 24),
        ...kLanguages.map((l) {
                ...
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        KsText(l['native']!, style: GoogleFonts.notoSans(fontSize: 17, fontWeight: FontWeight.w600)),
                        KsText(l['code'] == 'en' ? 'English' : 'KaamSetu language',
                            style: GoogleFonts.notoSans(fontSize: 12, color: AppTheme.mutedForeground)),
                      ],
                    ),
                  ),
                  if (selected) const Icon(Icons.verified_user, color: AppTheme.primary, size: 20),
                ],
              ),
            ),
          );
  ...
  Widget _buildRoleStep() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        KsText(t('iAmA'), style: GoogleFonts.notoSans(fontSize: 24, fontWeight: FontWeight.w700)),
        const SizedBox(height: 4),
        KsText('You can switch later from your profile.',
            style: GoogleFonts.notoSans(fontSize: 14, color: AppTheme.mutedForeground)),
        const SizedBox(height: 24),
        for (final r in [
          {'key': 'worker', 'icon': Icons.construction, 'title': t('worker'), 'sub': t('workerSub')},
          {'key': 'household', 'icon': Icons.home, 'title': t('household'), 'sub': t('householdSub')},
        ])
          GestureDetector(
              ...
              child: Row(
                children: [
                  AnimatedContainer(
                    ...
                    height: 48,
                    decoration: BoxDecoration(
                      color: _role == r['key'] ? AppTheme.primary : AppTheme.muted,
                      borderRadius: BorderRadius.circular(12),
                    ),
                    child: Icon(r['icon'] as IconData,
                        color: _role == r['key'] ? Colors.white : AppTheme.foreground,
                        size: 24),
                  ),
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        KsText(r['title'] as String,
                            style: GoogleFonts.notoSans(fontSize: 17, fontWeight: FontWeight.w700)),
                        KsText(r['sub'] as String,
  ...
  Widget _buildPhoneStep() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        KsText(t('phoneNumber'), style: GoogleFonts.notoSans(fontSize: 24, fontWeight: FontWeight.w700)),
        const SizedBox(height: 4),
        KsText("We send a one-time code to confirm it's you.",
            style: GoogleFonts.notoSans(fontSize: 14, color: AppTheme.mutedForeground)),
        const SizedBox(height: 24),
        Container(
        ...
        Container(
          ...
          child: KsText(
            ...
            style: GoogleFonts.notoSans(fontSize: 12, color: AppTheme.secondaryForeground),
          ),
        ),
      ],
    );
  ...
  Widget _buildOtpStep() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        KsText(t('enterOtp'), style: GoogleFonts.notoSans(fontSize: 24, fontWeight: FontWeight.w700)),
        const SizedBox(height: 4),
        RichText(
          text: TextSpan(
            style: GoogleFonts.notoSans(fontSize: 14, color: AppTheme.mutedForeground),
