...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  List<String> get _fullStepIds => widget.role == 'worker'
      ? [
          'identity',
          'area',
          'aadhaar',
  ...
  int get _progressPercent => ((_step + 1) / _stepIds.length * 100).round();

  String get _progressMessage {
    if (widget.quickSignUp) {
      return 'That’s it — add the rest whenever you are ready.';
    }
    if (_isLast) return 'Almost there — secure your account to finish.';
    if (_progressPercent >= 70)
      return 'You are nearly done — just a few details left.';
    if (_progressPercent >= 40)
      return 'Great progress — your profile is taking shape.';
    return 'A few quick steps now make finding the right match easier.';
  }
  ...
  void initState() {
    ...
    _lang = widget.lang;
    if (widget.profileDetailsOnly) {
      _name = context.read<AppProvider>().user?.name ?? '';
    }
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        context.read<AppProvider>().setLang(_lang);
      }
    });
  }
  ...
  Future<void> _selectAvailabilityTime({required bool isFrom}) async {
    final current = isFrom ? _availFrom : _availTo;
  ...
  bool get _canAdvance {
    switch (_current) {
      ...
      case 'availability':
        return true;
      case 'family':
        return _familyName.trim().length >= 2 &&
            _familyPhone.replaceAll(RegExp(r'\D'), '').length == 10;
      case 'pin':
        return RegExp(r'^\d{4}$').hasMatch(_pin) && _pin == _pin2;
      default:
        return true;
    }
  }
  ...
  void _verifyAadhaar() {
    if (_aadhaarStep == 1) {
      ...
      // Demo eKYC: every valid 12-digit identity number is accepted.
  ...
  Future<void> _submit() async {
    ...
    await provider.completeOnboarding(
      ...
      aadhaarLast4: _aadhaarVerified
          ? _aadhaar.replaceAll(RegExp(r'\D'), '').substring(8)
          : null,
      pin: _pin,
      radiusKm: _radiusKm,
      skills: _skills,
              ...
              Row(
                children: [
                  ...
                  AnimatedSwitcher(
                    ...
                    child: KsText(
                      'Step ${_step + 1} of ${_stepIds.length}',
                      key: ValueKey(_step),
                      ...
                      child: KsPrimaryButton(
                        label: _isLast
                            ? (widget.quickSignUp
                                  ? 'Create my account'
                                  : 'Finish setup')
                            : (_current == 'aadhaar' && _aadhaarStep < 3
                                  ? 'Verify identity'
                                  : t('continue')),
                        onTap: _canAdvance
                            ? (_isLast
  ...
  Widget _buildJourneyCard() {
    final remaining = _stepIds.length - _step - 1;
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Icon(
                widget.quickSignUp ? Icons.bolt_rounded : Icons.auto_awesome,
                color: AppTheme.primary,
                size: 18,
              ),
              const SizedBox(width: 8),
              Expanded(
                child: KsText(
                  widget.quickSignUp
                      ? 'Set up in seconds. Details can wait.'
                      : _progressMessage,
                  style: GoogleFonts.notoSans(
          ...
          if (!widget.quickSignUp && remaining > 0) ...[
            const SizedBox(height: 7),
            KsText(
              '$remaining quick ${remaining == 1 ? 'step' : 'steps'} left',
              style: GoogleFonts.notoSans(
                color: AppTheme.mutedForeground,
                fontSize: 11,
                fontWeight: FontWeight.w600,
  ...
  Widget _buildIdentity() {
    return Column(
      ...
      children: [
        ...
        KsText(
          "This is the name households and workers will see.",
          style: GoogleFonts.notoSans(
            fontSize: 13,
            color: AppTheme.mutedForeground,
          ),
  ...
  Widget _buildArea() {
    return Column(
      ...
      children: [
        ...
        KsText(
          widget.role == 'worker'
              ? "We only show you jobs inside your travel radius."
              : "We only show you workers who can reach you.",
          style: GoogleFonts.notoSans(
            fontSize: 13,
            color: AppTheme.mutedForeground,
          ),
        ...
        Container(
          ...
          child: Row(
            ...
            children: [
              ...
              Expanded(
                child: KsText(
                  'Demo: any 12 digits will pass this eKYC check. You can skip now and verify later from your profile.',
                  style: GoogleFonts.notoSans(
                    fontSize: 12,
                    color: AppTheme.secondaryForeground,
                    height: 1.4,
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                ...
                const SizedBox(width: 8),
                KsText(
                  'XXXX XXXX ${_aadhaar.replaceAll(RegExp(r'\D'), '').substring(8)}',
                  style: GoogleFonts.notoSans(
                    fontSize: 14,
                    fontWeight: FontWeight.w700,
                    color: AppTheme.success,
      ...
      children: [
        ...
        GridView.count(
          ...
          children: kSkills.map((s) {
            ...
            return GestureDetector(
              onTap: () => setState(() {
                on ? _skills.remove(s.key) : _skills.add(s.key);
              }),
              child: AnimatedContainer(
                duration: const Duration(milliseconds: 150),
                padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                    ...
                    Expanded(
                      child: KsText(
                        ...
                        style: GoogleFonts.notoSans(
                          fontSize: 12,
                          fontWeight: FontWeight.w600,
                          color: on ? AppTheme.primary : AppTheme.foreground,
                        ),
                        overflow: TextOverflow.ellipsis,
                      ),
                    ),
  ...
  Widget _buildAvailability() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        KsText(
          "Households see these hours on your profile.",
          style: GoogleFonts.notoSans(
            fontSize: 13,
            color: AppTheme.mutedForeground,
          ),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Material(
                      ...
                      child: InkWell(
                        onTap: () => _selectAvailabilityTime(
                          isFrom: f['isFrom'] as bool,
                        ),
                        borderRadius: BorderRadius.circular(12),
                        child: Container(
        ...
        TextField(
          ...
          decoration: const InputDecoration(
            hintText: 'e.g. 8 years of house painting. I bring my own tools.',
            labelText: 'About your work (optional)',
          ),
        ),
      ],
    );
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            Expanded(
              child: KsText(
                "Every job you take or post sends this person an SMS with the address and time.",
                style: GoogleFonts.notoSans(
                  fontSize: 13,
                  color: AppTheme.mutedForeground,
                  height: 1.5,
        ...
        TextField(
          ...
          maxLength: 10,
          onChanged: (v) => setState(() {
            _familyPhone = v.replaceAll(RegExp(r'\D'), '');
          }),
          decoration: const InputDecoration(
  ...
  Widget _buildPin() {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        KsText(
          "Your earnings and payments are protected by this PIN.",
          style: GoogleFonts.notoSans(
            fontSize: 13,
            color: AppTheme.mutedForeground,
          ),
