...
class JobsService {
    static async createJob(householdId, data) {
        ...
        try {
            const workers = await database_1.default.user.findMany({
    ...
    static async getJobById(jobId) {
        ...
        if (!job || job.deletedAt)
            throw new api_error_1.ApiError(404, 'JOB_NOT_FOUND', 'The requested job does not exist.');
        return job;
    }
    static async cancelJob(jobId, userId) {
        ...
        if (job.householdId !== userId)
            throw new api_error_1.ApiError(403, 'FORBIDDEN', 'You can only cancel your own jobs.');
        if (job.status !== 'OPEN' && job.status !== 'ASSIGNED') {
            throw new api_error_1.ApiError(400, 'INVALID_STATUS', 'Only OPEN or ASSIGNED jobs can be cancelled.');
        }
        return database_1.default.job.update({ where: { id: jobId }, data: { status: 'CANCELLED' } });
    }
    static async getMyPosts(userId) {
        return database_1.default.job.findMany({
    ...
    static async updateJobStatus(jobId, userId, status) {
        const job = await database_1.default.job.findUnique({ where: { id: jobId } });
        if (!job)
            throw new api_error_1.ApiError(404, 'JOB_NOT_FOUND', 'Job not found.');
        ...
        if (!allowed.includes(status)) {
            throw new api_error_1.ApiError(400, 'INVALID_TRANSITION', `Cannot transition from ${job.status} to ${status}.`);
        }
        if (status === 'CANCELLED' && job.householdId !== userId) {
            throw new api_error_1.ApiError(403, 'FORBIDDEN', 'Only the household can cancel a job.');
        }
        return database_1.default.job.update({ where: { id: jobId }, data: { status: status } });
    }
    static async applyForJob(jobId, workerId) {
        const job = await database_1.default.job.findUnique({ where: { id: jobId } });
        if (!job)
            throw new api_error_1.ApiError(404, 'JOB_NOT_FOUND', 'Job not found.');
        if (job.status !== 'OPEN')
            throw new api_error_1.ApiError(400, 'JOB_NOT_OPEN', 'Job is no longer open.');
        const interest = await database_1.default.jobInterest.upsert({
            where: { jobId_workerId: { jobId, workerId } },
            create: { jobId, workerId },
            update: {},
        });
        if (job.householdId) {
            const household = await database_1.default.user.findUnique({ where: { id: job.householdId } });
    ...
    static async getApplicants(jobId, householdId) {
        const job = await database_1.default.job.findUnique({ where: { id: jobId } });
        if (!job)
            throw new api_error_1.ApiError(404, 'JOB_NOT_FOUND', 'Job not found.');
        // Allow admin to see applicants too
        const user = await database_1.default.user.findUnique({ where: { id: householdId } });
        if (job.householdId !== householdId && user?.role !== 'ADMIN') {
            throw new api_error_1.ApiError(403, 'FORBIDDEN', 'You can only view applicants for your own jobs.');
        }
    ...
    static async assignWorker(jobId, householdId, workerId) {
        const job = await database_1.default.job.findUnique({ where: { id: jobId } });
        if (!job)
            throw new api_error_1.ApiError(404, 'JOB_NOT_FOUND', 'Job not found.');
        const user = await database_1.default.user.findUnique({ where: { id: householdId } });
        if (job.householdId !== householdId && user?.role !== 'ADMIN') {
            throw new api_error_1.ApiError(403, 'FORBIDDEN', 'You can only assign workers for your own jobs.');
        }
        if (job.status !== 'OPEN')
            throw new api_error_1.ApiError(400, 'JOB_NOT_OPEN', 'Job is no longer open.');
        const interest = await database_1.default.jobInterest.findUnique({ where: { jobId_workerId: { jobId, workerId } } });
        if (!interest)
            throw new api_error_1.ApiError(400, 'WORKER_NOT_APPLIED', 'Worker has not applied for this job.');
        const updatedJob = await database_1.default.job.update({
            where: { id: jobId },
            data: {
                status: 'ASSIGNED',
        ...
        // Accept the interest
        await database_1.default.jobInterest.update({
            where: { id: interest.id },
            data: { status: 'ACCEPTED' }
        });
        // Reject other interests
        await database_1.default.jobInterest.updateMany({
            where: { jobId, id: { not: interest.id } },
            data: { status: 'REJECTED' }
        });
        // Send push notification to the assigned worker
        const worker = await database_1.default.user.findUnique({ where: { id: workerId } });
        if (worker?.fcmToken) {
            await (0, fcm_1.sendPushNotification)({
                fcmToken: worker.fcmToken,
